    ...
    <div className="panel grid gap-3 p-4">
      ...
      {!readOnly && <div className="grid gap-3">
        ...
        {paymentRows.map((payment, index) => <div className="grid gap-2 rounded-md border border-slate-200 p-3 md:grid-cols-[160px_160px_1fr_44px]" key={index}>
          ...
          {(payment.method === "DEBIT" || payment.method === "CREDIT") ? <div className="grid gap-2 md:grid-cols-4"><input className="field" placeholder="Bandeira" value={payment.cardBrand} onChange={(event) => updatePaymentRow(index, { cardBrand: event.target.value })} /><input className="field" placeholder="NSU" value={payment.cardNsu} onChange={(event) => updatePaymentRow(index, { cardNsu: event.target.value })} /><input className="field" placeholder="Autorização" value={payment.cardAuthorization} onChange={(event) => updatePaymentRow(index, { cardAuthorization: event.target.value })} /><input className="field" placeholder="Parcelas" inputMode="numeric" value={payment.installments} onChange={(event) => updatePaymentRow(index, { installments: onlyDigits(event.target.value, 2) })} /></div> : <p className="self-center text-xs text-slate-500">Use mais de uma linha para pagamento misto.</p>}
