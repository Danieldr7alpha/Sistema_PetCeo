...
function FinancialModule({ sectionPage }: { sectionPage: string }) {
  ...
  if (sectionPage === "financial:accounts") return <Page title="Bancos" action={<button className="btn btn-primary" onClick={() => { setEditingAccount(null); setAccountOpen(true); }}><Plus size={16} />Cadastrar banco</button>}>
    <p className="mb-4 text-sm text-slate-600">Cadastre as contas bancárias onde serão recebidos PIX, cartões e valores das maquininhas. O Caixa físico é mantido pelo sistema.</p>
    <DataCards items={(accounts ?? []).map((account) => ({ title: account.name, subtitle: `${financialAccountTypeLabels[account.type]}${account.institutionName ? ` · ${account.institutionName}` : ""}`, meta: `Saldo inicial: ${currency(account.openingBalance)} | Saldo calculado: ${currency(account.calculatedBalance ?? account.openingBalance)}${account.isPrimary ? " | Conta principal" : ""}`, status: "Ativa", action: <div className="flex flex-wrap gap-2"><button className="btn btn-secondary" onClick={() => { setEditingAccount(account); setAccountOpen(true); }}>Editar</button>{account.type !== "CASH_DRAWER" && <button className="btn btn-secondary text-red-700" onClick={async () => { if (!window.confirm(`Excluir o banco ${account.name}?`)) return; await api(`/financial/accounts/${account.id}`, { method: "DELETE" }); refreshAccounts(); refreshMethods(); }}>Excluir</button>}</div> }))} />
...
function FinancialPayables({ accounts, openTotal, onChanged }: { accounts: FinancialAccount[]; openTotal: number; onChanged: () => void }) {
  ...
  const { data: payables, refresh } = useData<FinancialPayable[]>(`/financial/payables?status=${status}`);
  ...
  return <Page title="Contas a pagar" action={<button className="btn btn-primary" onClick={() => { setEditing(null); setFormOpen(true); }}><Plus size={16} />Nova conta</button>}>
    <div className="mb-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-red-200 bg-red-50 p-4"><p className="text-sm text-red-700">Total em aberto</p><b className="text-xl text-red-950">{currency(openTotal)}</b></div><div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm text-amber-700">Contas vencidas</p><b className="text-xl text-amber-950">{overdue}</b></div><div className="panel p-4"><label className="text-sm font-medium">Exibir<select className="field mt-1" value={status} onChange={(e) => setStatus(e.target.value)}><option value="OPEN">Em aberto</option><option value="PAID">Pagas</option><option value="CANCELLED">Canceladas</option><option value="ALL">Todas</option></select></label></div></div>
...
function PayableModal({ payable, onClose, onSaved }: { payable: FinancialPayable | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={payable ? "Editar conta a pagar" : "Nova conta a pagar"} onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
    ...
    <label className="text-sm font-semibold">Categoria<select className="field mt-1" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>{Object.entries(payableCategoryLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
    ...
    {!payable && <label className="text-sm font-semibold">Quantas parcelas?<input className="field mt-1" required type="number" min="1" max="60" value={form.repeatMonths} onChange={(e) => setForm({ ...form, repeatMonths: e.target.value })} /><span className="mt-1 block text-xs font-normal text-slate-500">Digite 1 para pagamento único.</span></label>}
    <label className="text-sm font-semibold">Vencimento da primeira parcela<input className="field mt-1" required type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} /><span className="mt-1 block text-xs font-normal text-slate-500">As demais vencerão no mesmo dia de cada mês.</span></label>
    <label className="text-sm font-semibold">Mês de referência<input className="field mt-1" type="month" value={form.competenceMonth} onChange={(e) => setForm({ ...form, competenceMonth: e.target.value })} /></label>
