...
function PayableModal({ payable, onClose, onSaved }: { payable: FinancialPayable | null; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ description: payable?.description ?? "", category: payable?.category ?? "RENT", expenseType: payable?.expenseType ?? "FIXED", supplierName: payable?.supplierName ?? "", documentNumber: payable?.documentNumber ?? "", amount: String(payable?.amount ?? ""), dueDate: payable?.dueDate?.slice(0, 10) ?? localDateInput(), competenceMonth: payable?.competenceMonth ?? localDateInput().slice(0, 7), recurring: false, repeatMonths: "12", notes: payable?.notes ?? "" });
  const [error, setError] = useState(""); const [saving, setSaving] = useState(false);
  async function submit(event: React.FormEvent) { event.preventDefault(); setSaving(true); setError(""); try { await api(payable ? `/financial/payables/${payable.id}` : "/financial/payables", { method: payable ? "PATCH" : "POST", body: JSON.stringify({ ...form, amount: Number(form.amount.replace(",", ".")), repeatMonths: Number(form.repeatMonths) }) }); onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível salvar a conta."); } finally { setSaving(false); } }
  return <Modal title={payable ? "Editar conta a pagar" : "Nova conta a pagar"} onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}><label className="text-sm font-semibold sm:col-span-2">Descrição da conta<input className="field mt-1" required placeholder="Ex.: Aluguel da loja, compra de shampoos" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label><label className="text-sm font-semibold">Categoria<select className="field mt-1" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>{Object.entries(payableCategoryLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label className="text-sm font-semibold">Tipo da despesa<select className="field mt-1" value={form.expenseType} onChange={(e) => setForm({ ...form, expenseType: e.target.value as "FIXED" | "VARIABLE" })}><option value="FIXED">Fixa — repete normalmente</option><option value="VARIABLE">Variável — valor ou ocorrência muda</option></select></label><label className="text-sm font-semibold">Fornecedor ou favorecido <span className="font-normal text-slate-500">(opcional)</span><input className="field mt-1" placeholder="Ex.: Distribuidora Pet" value={form.supplierName} onChange={(e) => setForm({ ...form, supplierName: e.target.value })} /></label><label className="text-sm font-semibold">Nota fiscal, boleto ou documento <span className="font-normal text-slate-500">(opcional)</span><input className="field mt-1" value={form.documentNumber} onChange={(e) => setForm({ ...form, documentNumber: e.target.value })} /></label><label className="text-sm font-semibold">Valor (R$)<input className="field mt-1" required inputMode="decimal" placeholder="0,00" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} /></label><label className="text-sm font-semibold">Data de vencimento<input className="field mt-1" required type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} /></label><label className="text-sm font-semibold">Mês de competência<input className="field mt-1" type="month" value={form.competenceMonth} onChange={(e) => setForm({ ...form, competenceMonth: e.target.value })} /></label>{!payable && <div className="rounded-lg border border-slate-200 p-3"><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={form.recurring} onChange={(e) => setForm({ ...form, recurring: e.target.checked })} />Repetir mensalmente</label>{form.recurring && <label className="mt-3 block text-sm font-semibold">Quantidade de meses<select className="field mt-1" value={form.repeatMonths} onChange={(e) => setForm({ ...form, repeatMonths: e.target.value })}>{[2,3,6,12,18,24,36,48,60].map((number) => <option key={number}>{number}</option>)}</select></label>}</div>}<label className="text-sm font-semibold sm:col-span-2">Observações <span className="font-normal text-slate-500">(opcional)</span><textarea className="field mt-1" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></label>{error && <p className="rounded-md bg-red-50 p-3 text-sm text-red-700 sm:col-span-2">{error}</p>}<button className="btn btn-primary justify-self-end sm:col-span-2" disabled={saving}>{saving ? "Salvando..." : "Salvar conta"}</button></form></Modal>;
}
...
function PayablePaymentModal({ payable, accounts, onClose, onSaved }: { payable: FinancialPayable; accounts: FinancialAccount[]; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ paidAmount: String(payable.amount), paidAt: localDateInput(), paidFromAccountId: accounts[0]?.id ?? "", paymentMethod: "PIX" }); const [error, setError] = useState("");
  async function submit(event: React.FormEvent) { event.preventDefault(); setError(""); try { await api(`/financial/payables/${payable.id}/pay`, { method: "POST", body: JSON.stringify({ ...form, paidAmount: Number(form.paidAmount.replace(",", ".")) }) }); onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível registrar o pagamento."); } }
  return <Modal title="Registrar pagamento" onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}><div className="rounded-lg bg-slate-50 p-3 sm:col-span-2"><b>{payable.description}</b><p>Valor da conta: {currency(payable.amount)}</p></div><label className="text-sm font-semibold">Valor pago<input className="field mt-1" required inputMode="decimal" value={form.paidAmount} onChange={(e) => setForm({ ...form, paidAmount: e.target.value })} /></label><label className="text-sm font-semibold">Data do pagamento<input className="field mt-1" required type="date" value={form.paidAt} onChange={(e) => setForm({ ...form, paidAt: e.target.value })} /></label><label className="text-sm font-semibold">Pago com qual conta?<select className="field mt-1" required value={form.paidFromAccountId} onChange={(e) => setForm({ ...form, paidFromAccountId: e.target.value })}>{accounts.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select></label><label className="text-sm font-semibold">Forma de pagamento<select className="field mt-1" value={form.paymentMethod} onChange={(e) => setForm({ ...form, paymentMethod: e.target.value })}><option>PIX</option><option>Dinheiro</option><option>Débito automático</option><option>Boleto</option><option>Transferência</option><option>Cartão</option><option>Outro</option></select></label>{error && <p className="rounded-md bg-red-50 p-3 text-sm text-red-700 sm:col-span-2">{error}</p>}<button className="btn btn-primary justify-self-end sm:col-span-2">Confirmar pagamento</button></form></Modal>;
}
