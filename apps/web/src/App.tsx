...
function CustomerForm({ customerId, onClose, onSaved }: { customerId?: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function createRegistrationLink() {
    ...
    try {
      const result = await api<{ url: string }>("/customers/registration-link", { method: "POST" });
  ...
  return <Modal title={isEdit ? "Editar Cliente" : "Adicionar Cliente"} onClose={onClose}><form onSubmit={submit} className="grid gap-4">
    ...
    {!isEdit && <section className="grid gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4"><div><h3 className="font-semibold text-blue-950">Cliente pode preencher o cadastro</h3><p className="text-sm text-blue-800">Gere um link seguro e envie pelo WhatsApp. O link vale por 7 dias e só pode ser usado uma vez.</p></div>{!registrationLink ? <button className="btn btn-secondary w-full sm:w-fit" type="button" disabled={creatingLink} onClick={createRegistrationLink}>{creatingLink ? "Gerando link..." : "Enviar link para o cliente"}</button> : <div className="grid gap-2"><input className="field" readOnly value={registrationLink} /><div className="grid grid-cols-2 gap-2"><button className="btn btn-secondary" type="button" onClick={async () => { await navigator.clipboard.writeText(registrationLink); }}>Copiar link</button><button className="btn btn-primary" type="button" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(`Preencha seu cadastro no CEO Pet AI: ${registrationLink}`)}`, "_blank")}>Enviar no WhatsApp</button></div></div>}</section>}
