  ...
  return <div className="fixed inset-0 z-[100] bg-slate-950/30" onClick={onClose}>
    <aside className="ml-auto flex h-full w-full max-w-md flex-col bg-white shadow-xl" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="grid gap-4 overflow-y-auto p-5 text-sm">
        ...
        {canCharge && <section className="rounded-xl border-2 border-blue-300 bg-blue-50 p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-blue-950">Pagamento pendente</h3><p className="mt-1 text-sm text-blue-800">O atendimento foi finalizado e o pedido está pronto para cobrança.</p></div><span className="badge shrink-0 bg-white text-blue-800">{saleCode(order as Pick<Sale, "internalCode">)}</span></div><div className="mt-3 rounded-lg bg-white p-3 text-sm text-slate-700"><p><b>Valor a receber:</b> {currency(chargeableTotal)}</p><p className="mt-1 text-xs text-slate-600">Você pode usar o botão abaixo ou digitar este número de pedido diretamente no Caixa.</p></div><button disabled={loadingId === appointment.id} className="btn btn-primary mt-3 w-full disabled:opacity-40" onClick={() => onCharge(appointment)}>{loadingId === appointment.id ? "Abrindo caixa..." : "Ir para o Caixa"}</button></section>}
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {appointment.status === "FINISHED" && order?.status === "PAID" ? <span className="badge bg-emerald-50 text-emerald-700">Pagamento concluído</span> : noChargeRequired ? <span className="badge bg-emerald-50 text-emerald-700">Sem cobrança pendente</span> : null}
