...
function FinancialModule({ sectionPage }: { sectionPage: string }) {
  ...
  if (sectionPage === "financial:accounts") return <Page title="Bancos" action={<button className="btn btn-primary" onClick={() => setAccountOpen(true)}><Plus size={16} />Cadastrar banco</button>}>
...
function FinancialAccountModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ name: "", type: "CHECKING_ACCOUNT", institutionName: "", agency: "", accountNumber: "", internalIdentifier: "", openingBalance: "0", openingBalanceDate: localDateInput(), isPrimary: false, active: true, notes: "" });
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  ...
  return <Modal title="Cadastrar banco" onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
    ...
    <label className="text-sm font-semibold">Banco ou instituição<input className="field mt-1" required placeholder="Ex.: Santander, Itaú, Nubank" value={form.institutionName} onChange={(e) => setForm({ ...form, institutionName: e.target.value })} /></label>
    <label className="text-sm font-semibold">Tipo de conta<select className="field mt-1" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>{Object.entries(financialAccountTypeLabels).filter(([value]) => value !== "CASH_DRAWER").map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
    ...
    <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isPrimary} onChange={(e) => setForm({ ...form, isPrimary: e.target.checked })} />Usar como conta bancária principal</label>
    ...
    {error && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 sm:col-span-2">{error}</p>}<button className="btn btn-primary justify-self-end sm:col-span-2" disabled={saving}>{saving ? "Salvando..." : "Cadastrar banco"}</button>
  </form></Modal>;
}
