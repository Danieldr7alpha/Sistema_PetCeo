...
function FinancialModule({ sectionPage }: { sectionPage: string }) {
  ...
  if (sectionPage === "financial:methods") return <Page title="Formas de recebimento" action={<button className="btn btn-primary" disabled={!accounts?.some((account) => account.active)} onClick={() => { setEditingMethod(null); setMethodOpen(true); }}><Plus size={16} />Nova forma</button>}>
...
function FinancialMethodModal({ method, accounts, onClose, onSaved }: { method: FinancialPaymentMethod | null; accounts: FinancialAccount[]; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ name: method?.name ?? "", type: method?.type ?? "PIX", institutionName: method?.institutionName ?? "", destinationAccountId: method?.destinationAccountId ?? accounts[0]?.id ?? "", maxInstallments: method?.maxInstallments ?? 1, brands: method?.brands.filter((brand) => brand.active).map((brand) => brand.brandName).join(", ") ?? "", settlementContract: method?.settlementContract ?? "SCHEDULED", settlementDays: String(method?.settlementDays ?? 1), settlementDayType: method?.settlementDayType ?? "BUSINESS" });
  ...
  return <Modal title={method ? `Editar — ${method.name}` : "Nova forma de pagamento"} onClose={onClose}><form className="grid gap-5" onSubmit={submit}>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-semibold">Tipo de pagamento<select className="field mt-1" disabled={isCash} value={form.type} onChange={(e) => { const type = e.target.value as FinancialPaymentMethod["type"]; setForm({ ...form, type, maxInstallments: 1 }); setRates([{ fee: "", anticipates: false, anticipationFee: "" }]); }}>{isCash && <option value="CASH">Dinheiro</option>}<option value="PIX">PIX</option><option value="DEBIT_CARD">Cartão de débito</option><option value="CREDIT_CARD">Cartão de crédito</option></select></label>
