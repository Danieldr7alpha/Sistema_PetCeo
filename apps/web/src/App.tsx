...
type CashRegister = { id: string; code: string; name: string; initialBalance: number | string; balance?: number; active: boolean; notes?: string | null; allowedUsers?: { id: string; name: string; active: boolean }[]; openSession?: { id: string; openedByName?: string; openedAt: string } | null };
type CashSession = { id: string; internalCode?: number; cashRegisterId?: string | null; cashRegister?: CashRegister | null; status: "OPEN" | "CLOSED"; openedByName?: string; closedByName?: string; openingAmount: number | string; closingCashAmount?: number | string | null; expectedCashAmount?: number | string | null; difference?: number | string | null; differenceReason?: string | null; notes?: string | null; openedAt: string; closedAt?: string | null; movements?: CashMovement[] };
...
type FinancialAccount = { id: string; name: string; type: "CASH_DRAWER" | "CHECKING_ACCOUNT" | "SAVINGS_ACCOUNT" | "DIGITAL_ACCOUNT" | "PAYMENT_WALLET" | "OTHER"; institutionName?: string | null; agency?: string | null; accountNumber?: string | null; internalIdentifier?: string | null; openingBalance: number | string; openingBalanceDate: string; calculatedBalance?: number; isPrimary: boolean; active: boolean; notes?: string | null };
...
function appointmentColor(status: string) {
  if (status === "ARRIVED") return { dot: "bg-orange-500", card: "border-orange-300 border-l-4 border-l-orange-500 bg-orange-50" };
  if (status === "IN_SERVICE" || status === "BATHING") return { dot: "bg-violet-600", card: "border-violet-300 border-l-4 border-l-violet-600 bg-violet-50" };
  if (status === "WAITING_PICKUP") return { dot: "bg-cyan-600", card: "border-cyan-300 border-l-4 border-l-cyan-600 bg-cyan-50" };
  if (status === "FINISHED") return { dot: "bg-emerald-600", card: "border-emerald-300 border-l-4 border-l-emerald-600 bg-emerald-50" };
  if (status === "CANCELLED") return { dot: "bg-red-600", card: "border-red-300 border-l-4 border-l-red-600 bg-red-50" };
  return { dot: "bg-blue-600", card: "border-blue-300 border-l-4 border-l-blue-600 bg-blue-50" };
}
  ...
  if (mode === "login") return <main className="grid min-h-screen place-items-center bg-slate-100 p-4">
    <form onSubmit={submit} className="panel w-full max-w-md p-6 shadow-sm">
      ...
      <div className="mt-5 grid gap-3">
        ...
        <div className="relative"><input className="field pr-12" aria-label="Senha" autoComplete="current-password" placeholder="Senha" type={showLoginPassword ? "text" : "password"} required value={loginForm.password} onChange={(e) => setLoginForm({ ...loginForm, password: e.target.value })} /><button className="absolute inset-y-0 right-0 flex w-12 items-center justify-center text-slate-500 hover:text-slate-800" type="button" aria-label={showLoginPassword ? "Ocultar senha" : "Mostrar senha"} title={showLoginPassword ? "Ocultar senha" : "Mostrar senha"} onClick={() => setShowLoginPassword((current) => !current)}>{showLoginPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button></div>
      </div>
      {error && <p className="mt-3 rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}
      {message && <p className="mt-3 rounded-md bg-green-50 px-3 py-2 text-sm text-green-700">{message}</p>}
...
function CustomerForm({ customerId, onClose, onSaved }: { customerId?: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={isEdit ? "Editar Cliente" : "Adicionar Cliente"} onClose={onClose}><form onSubmit={submit} className="grid gap-4">
    ...
    <button className="btn btn-primary justify-self-end disabled:opacity-60" type="submit" disabled={saving}><Check size={16} />{saving ? "Salvando..." : isEdit ? "Salvar alterações" : "Concluir cadastro"}</button>
...
function Appointments({ onCharge, onRenewMembership }: { onCharge: (appointment: Appointment) => void; onRenewMembership: () => void }) {
  ...
  return <Page title="Agenda" action={<button className="btn btn-primary" onClick={() => openAt("09:00")}><Plus size={16} />Novo Agendamento</button>}>
    ...
    <div className={`panel overflow-hidden ${calendarExpanded ? "agenda-expanded fixed inset-0 z-[80] rounded-none border-0 bg-white" : ""}`}>
      {calendarExpanded ? <div className="grid grid-cols-[40px_minmax(0,1fr)_40px_auto_40px] items-center gap-1 border-b border-slate-200 bg-white px-2 py-2 shadow-sm sm:grid-cols-[44px_minmax(180px,1fr)_44px_auto_44px] sm:gap-2 sm:px-4">
        <button className="btn btn-secondary !min-h-10 !p-2" aria-label="Semana anterior" onClick={() => shiftPeriod(-1)}>◀</button>
        <span className="truncate text-center text-sm font-semibold capitalize">{periodTitle("week", selectedDate)}</span>
        <button className="btn btn-secondary !min-h-10 !p-2" aria-label="Próxima semana" onClick={() => shiftPeriod(1)}>▶</button>
        <span className="whitespace-nowrap px-1 text-xs font-semibold text-slate-700 sm:text-sm">🐶 {periodAppointments.length}</span>
        <button className="btn btn-secondary !min-h-10 !p-2" type="button" title="Restaurar tela normal" aria-label="Restaurar tela normal" onClick={() => setCalendarExpanded(false)}><Minimize2 size={18} /></button>
      </div> : <div className="flex flex-col gap-3 border-b border-slate-200 p-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      {view === "day" && <><div className="hidden max-h-[70vh] overflow-y-auto md:block">{hours.map((hour) => { const slot = `${selectedDate}-${hour}`; return <div className="grid min-h-24 grid-cols-[72px_1fr] border-b border-slate-300" key={hour}><button className="border-r border-slate-300 bg-slate-50 p-3 text-left text-sm font-bold text-slate-800 hover:bg-slate-200" onClick={() => openAt(hour)}>{hour}</button><div {...dropZoneProps(selectedDate, hour)} className={`grid grid-cols-1 gap-2 bg-white p-3 transition-colors hover:bg-blue-50 xl:grid-cols-2 2xl:grid-cols-3 ${dragOverSlot === slot ? "bg-blue-100 ring-2 ring-inset ring-blue-600" : ""}`} onClick={(event) => { if (event.currentTarget === event.target) openAt(hour); }}>{dayAppointments(selectedDate).filter((appointment) => appointment.startTime.slice(0, 2) === hour.slice(0, 2)).map((appointment) => appointmentCard(appointment))}</div></div>; })}</div><div className="grid gap-3 p-3 md:hidden">{dayAppointments(selectedDate).map((appointment) => appointmentCard(appointment, true))}{!dayAppointments(selectedDate).length && <p className="rounded-lg border border-dashed border-slate-300 p-4 text-center text-sm text-slate-600">Nenhum atendimento encontrado para este dia.</p>}</div></>}
      {view === "week" && <><div className="week-calendar-grid hidden max-h-[70vh] overflow-auto md:block"><div className="week-calendar-header week-calendar-row sticky top-0 z-20 grid grid-cols-[56px_repeat(7,minmax(108px,1fr))] border-b border-slate-200 bg-slate-50 shadow-sm">{["", ...weekDays(selectedDate)].map((day, index) => <div className={`${index === 0 ? "sticky left-0 z-30" : ""} bg-slate-50 p-2 text-center text-xs font-semibold capitalize`} key={index}>{day && parseLocalDate(day).toLocaleDateString("pt-BR", { weekday: "short", day: "2-digit" })}</div>)}</div>{hours.map((hour) => <div className="week-calendar-row grid grid-cols-[56px_repeat(7,minmax(108px,1fr))] border-b border-slate-100" key={hour}><div className="sticky left-0 z-[5] border-r border-slate-100 bg-white p-2 text-xs font-semibold text-slate-700">{hour}</div>{weekDays(selectedDate).map((day) => { const slot = `${day}-${hour}`; return <div {...dropZoneProps(day, hour)} className={`min-h-24 border-r border-slate-100 p-1.5 text-left hover:bg-slate-50 ${day === localDateInput() ? "bg-blue-50/50" : ""} ${dragOverSlot === slot ? "bg-blue-100 ring-2 ring-inset ring-blue-400" : ""}`} key={`${day}-${hour}`} onClick={(event) => { if (event.currentTarget === event.target) openAt(hour, day); }}>{dayAppointments(day).filter((appointment) => appointment.startTime.slice(0, 2) === hour.slice(0, 2)).map((appointment) => appointmentCard(appointment, true))}</div>; })}</div>)}</div><div className="week-mobile-list grid gap-3 p-3 md:hidden">{weekDays(selectedDate).map((day) => <section className="grid gap-2" key={day}><h3 className="font-semibold capitalize">{parseLocalDate(day).toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "2-digit" })}</h3>{dayAppointments(day).map((appointment) => appointmentCard(appointment, true))}<button className="btn btn-secondary justify-self-start" onClick={() => openAt("09:00", day)}>Novo neste dia</button></section>)}</div></>}
...
function CashPointOfSaleLayout({ cashSession, query, setQuery, loadedSale, waitingSales, pendingSales, onOpened, onShortcut, onOpenSale, onSaved, session }: { cashSession: CashSession | null; query: string; setQuery: (value: string) => void; loadedSale: Sale | null; waitingSales: Sale[]; pendingSales: Sale[]; onOpened: () => void; onShortcut: (section: CashSectionKey) => void; onOpenSale: (sale: Sale) => void; onSaved: () => void; session: Session }) {
  const { data: availableRegisters } = useData<CashRegister[]>(cashSession ? "" : "/cash/registers");
  ...
  useEffect(() => { if (!cashRegisterId && availableRegisters?.length) setCashRegisterId(availableRegisters[0].id); }, [availableRegisters, cashRegisterId]);
  const { data: openingState } = useData<CashCurrent>(cashSession || !cashRegisterId ? "" : `/cash/current?cashRegisterId=${cashRegisterId}`, [cashSession?.id, cashRegisterId]);
  useEffect(() => { if (openingState?.hasPreviousSession) setOpeningCents(String(Math.round(Number(openingState.openingBalance) * 100))); }, [openingState?.hasPreviousSession, openingState?.openingBalance]);
  ...
  async function openCash() {
    setOpenError("");
    try {
      await api("/cash/open", { method: "POST", body: JSON.stringify({ openingAmount: decimalFromCents(openingCents), cashRegisterId }) });
      onOpened();
    } catch (error) {
      setOpenError(error instanceof Error ? error.message : "Falha ao abrir caixa.");
    }
  ...
  if (!cashSession) return <div className="panel grid gap-4 p-5 md:grid-cols-[1fr_320px] md:items-start">
    ...
    <div className="grid gap-3">
      <label className="text-sm font-medium">Caixa autorizado<select className="field mt-1" required value={cashRegisterId} onChange={(event) => setCashRegisterId(event.target.value)}><option value="">Selecione...</option>{(availableRegisters ?? []).map((register) => <option value={register.id} key={register.id}>{register.code} — {register.name}</option>)}</select></label>
      <label className="text-sm font-medium">Valor de abertura<input className="field mt-1" disabled={openingState?.hasPreviousSession} inputMode="numeric" value={formatCurrencyInput(openingCents)} onChange={(event) => setOpeningCents(onlyDigits(event.target.value))} /></label>
      {openingState?.hasPreviousSession && <p className="text-xs text-slate-500">Valor herdado automaticamente do último fechamento.</p>}
      {openError && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{openError}</p>}
      {!availableRegisters?.length && <p className="rounded-lg bg-amber-50 p-3 text-sm text-amber-800">Nenhum caixa foi autorizado para seu usuário. Solicite acesso ao administrador.</p>}
      <button className="btn btn-primary" disabled={!cashRegisterId} type="button" onClick={openCash}>Abrir caixa</button>
    </div>
  </div>;
...
function CashSessionPanel({ cashSession, onRefresh, onClosed }: { cashSession: CashSession | null; onRefresh: () => void; onClosed: (session: CashSession) => void }) {
  const [openingCents, setOpeningCents] = useState("0");
  const [notes, setNotes] = useState("");
  ...
  async function closeCash() {
    ...
    setMessage("");
    try {
      const closed = await api<CashSession>(`/cash/${cashSession.id}/close`, { method: "POST", body: JSON.stringify({ countedCashAmount: countedCash, differenceReason, adminPassword: closePassword }) });
      onClosed(closed);
      onRefresh();
    } catch (error) {
...
function CashReports({ isAdmin, cashSession, closingReport = false, onDismiss }: { isAdmin: boolean; cashSession: CashSession | null; closingReport?: boolean; onDismiss?: () => void }) {
  ...
  const [customTo, setCustomTo] = useState(localDateInput());
  const range = cashPeriodRange(period, customFrom, customTo);
  const reportParams = (closingReport || period === "today") && cashSession?.id
...
function TeamManagement() {
  const { data: users, refresh } = useData<TeamUser[]>("/management/team"); const { data: registers } = useData<CashRegister[]>("/management/cash-registers");
  const [editing, setEditing] = useState<TeamUser | null>(null); const [open, setOpen] = useState(false);
  return <Page title="Equipe" action={<button className="btn btn-primary" onClick={() => { setEditing(null); setOpen(true); }}><Plus size={16} />Cadastrar funcionário</button>}><p className="mb-4 text-sm text-slate-600">Cadastre os dados, os custos mensais e escolha quais áreas e caixas o funcionário poderá utilizar.</p><DataCards items={(users ?? []).map((user) => ({ title: user.name, subtitle: `${user.jobTitle || "Cargo não informado"} · ${user.email}`, meta: `${user.phone ? formatPhone(user.phone) : "Telefone não informado"} · Custo mensal: ${currency(Number(user.salary ?? 0) + Number(user.otherMonthlyCost ?? 0))} · ${user.role === "ADMIN" ? "Acesso completo" : `${user.permissions.length} permissão(ões)`}`, status: user.active ? "Ativo" : "Inativo", action: <div className="flex gap-2"><button className="btn btn-secondary" onClick={() => { setEditing(user); setOpen(true); }}>Editar</button>{user.role !== "ADMIN" && <button className="btn btn-secondary text-red-700" onClick={async () => { if (!window.confirm(`Desativar o acesso de ${user.name}?`)) return; await api(`/management/team/${user.id}`, { method: "DELETE" }); refresh(); }}>Desativar</button>}</div> }))} />{open && <TeamUserModal user={editing} registers={(registers ?? []).filter((register) => register.active)} onClose={() => { setOpen(false); setEditing(null); }} onSaved={() => { setOpen(false); setEditing(null); refresh(); }} />}</Page>;
}

function TeamUserModal({ user, registers, onClose, onSaved }: { user: TeamUser | null; registers: CashRegister[]; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ name: user?.name ?? "", cpf: user?.cpf ?? "", phone: user?.phone ?? "", jobTitle: user?.jobTitle ?? "", salaryCents: user?.salary ? String(Math.round(Number(user.salary) * 100)) : "", otherMonthlyCostCents: user?.otherMonthlyCost ? String(Math.round(Number(user.otherMonthlyCost) * 100)) : "", otherCostDescription: user?.otherCostDescription ?? "", zipCode: user?.zipCode ?? "", street: user?.street ?? "", addressNumber: user?.addressNumber ?? "", complement: user?.complement ?? "", neighborhood: user?.neighborhood ?? "", city: user?.city ?? "", state: user?.state ?? "", email: user?.email ?? "", password: "", role: user?.role ?? "EMPLOYEE", active: user?.active ?? true, permissions: user?.permissions ?? [], cashRegisterIds: user?.cashRegisterAccess.map((access) => access.cashRegisterId) ?? [] });
  const [error, setError] = useState(""); const [lookingUpCep, setLookingUpCep] = useState(false); const isAdmin = form.role === "ADMIN";
  const toggle = (field: "permissions" | "cashRegisterIds", value: string) => setForm((current) => ({ ...current, [field]: current[field].includes(value) ? current[field].filter((item) => item !== value) : [...current[field], value] }));
  async function lookupCep() { const cep = onlyDigits(form.zipCode, 8); if (cep.length !== 8) return; setLookingUpCep(true); try { const response = await fetch(`https://viacep.com.br/ws/${cep}/json/`); const data = await response.json(); if (data.erro) throw new Error(); setForm((current) => ({ ...current, street: data.logradouro ?? "", neighborhood: data.bairro ?? "", city: data.localidade ?? "", state: data.uf ?? "" })); } catch { setError("CEP não encontrado. Você pode preencher o endereço manualmente."); } finally { setLookingUpCep(false); } }
  async function submit(event: React.FormEvent) { event.preventDefault(); setError(""); try { const { salaryCents, otherMonthlyCostCents, ...values } = form; await api(user ? `/management/team/${user.id}` : "/management/team", { method: user ? "PATCH" : "POST", body: JSON.stringify({ ...values, cpf: onlyDigits(form.cpf, 11) || null, phone: onlyDigits(form.phone, 11) || null, zipCode: onlyDigits(form.zipCode, 8) || null, salary: salaryCents ? Number(salaryCents) / 100 : 0, otherMonthlyCost: otherMonthlyCostCents ? Number(otherMonthlyCostCents) / 100 : 0, password: form.password || undefined }) }); onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível salvar o funcionário."); } }
  return <Modal title={user ? "Editar funcionário" : "Cadastrar funcionário"} onClose={onClose}><form className="grid gap-6" onSubmit={submit}>
    <section><h3 className="font-semibold">Dados pessoais e profissionais</h3><p className="mb-3 text-sm text-slate-500">Informações do funcionário e custos mensais para a futura folha de pagamento.</p><div className="grid gap-4 sm:grid-cols-2">
      <label className="text-sm font-semibold">Nome completo<input className="field mt-1" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label>
      <label className="text-sm font-semibold">CPF<input className="field mt-1" required inputMode="numeric" placeholder="000.000.000-00" value={formatCpf(form.cpf)} onChange={(e) => setForm({ ...form, cpf: onlyDigits(e.target.value, 11) })} /></label>
      <label className="text-sm font-semibold">Telefone / WhatsApp<input className="field mt-1" required inputMode="tel" placeholder="(11) 99999-9999" value={formatPhone(form.phone)} onChange={(e) => setForm({ ...form, phone: onlyDigits(e.target.value, 11) })} /></label>
      <label className="text-sm font-semibold">Cargo na empresa<input className="field mt-1" required placeholder="Ex.: Tosador, recepcionista" value={form.jobTitle} onChange={(e) => setForm({ ...form, jobTitle: e.target.value })} /></label>
      <label className="text-sm font-semibold">Salário mensal<input className="field mt-1" required inputMode="numeric" value={formatCurrencyInput(form.salaryCents)} onChange={(e) => setForm({ ...form, salaryCents: onlyDigits(e.target.value) })} /></label>
      <label className="text-sm font-semibold">Outro custo mensal<input className="field mt-1" inputMode="numeric" value={formatCurrencyInput(form.otherMonthlyCostCents)} onChange={(e) => setForm({ ...form, otherMonthlyCostCents: onlyDigits(e.target.value) })} /></label>
      <label className="text-sm font-semibold sm:col-span-2">Descrição do outro custo<input className="field mt-1" placeholder="Ex.: Vale-transporte, ajuda de custo, benefícios" value={form.otherCostDescription} onChange={(e) => setForm({ ...form, otherCostDescription: e.target.value })} /></label>
    </div></section>
    <section><h3 className="font-semibold">Endereço</h3><p className="mb-3 text-sm text-slate-500">Digite o CEP para preencher automaticamente os outros campos.</p><div className="grid gap-4 sm:grid-cols-2">
      <label className="text-sm font-semibold">CEP<input className="field mt-1" inputMode="numeric" placeholder="00000-000" value={formatCep(form.zipCode)} onBlur={lookupCep} onChange={(e) => setForm({ ...form, zipCode: onlyDigits(e.target.value, 8) })} /></label>
      <label className="text-sm font-semibold">Rua<input className="field mt-1" value={form.street} onChange={(e) => setForm({ ...form, street: e.target.value })} /></label>
      <label className="text-sm font-semibold">Número<input className="field mt-1" value={form.addressNumber} onChange={(e) => setForm({ ...form, addressNumber: e.target.value })} /></label>
      <label className="text-sm font-semibold">Complemento<input className="field mt-1" value={form.complement} onChange={(e) => setForm({ ...form, complement: e.target.value })} /></label>
      <label className="text-sm font-semibold">Bairro<input className="field mt-1" value={form.neighborhood} onChange={(e) => setForm({ ...form, neighborhood: e.target.value })} /></label>
      <label className="text-sm font-semibold">Cidade<input className="field mt-1" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} /></label>
      <label className="text-sm font-semibold">Estado (UF)<input className="field mt-1" maxLength={2} value={form.state} onChange={(e) => setForm({ ...form, state: e.target.value.toUpperCase() })} /></label>{lookingUpCep && <p className="self-end text-sm text-blue-700">Buscando CEP...</p>}
    </div></section>
    <section><h3 className="font-semibold">Acesso ao sistema</h3><div className="mt-3 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">E-mail de acesso<input className="field mt-1" required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label className="text-sm font-semibold">{user ? "Nova senha (deixe vazia para manter)" : "Senha para entrar"}<input className="field mt-1" required={!user} minLength={6} type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></label><label className="text-sm font-semibold sm:col-span-2">Tipo de acesso<select className="field mt-1" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as "ADMIN" | "EMPLOYEE" })}><option value="EMPLOYEE">Funcionário — acesso personalizado</option><option value="ADMIN">Administrador — acesso completo</option></select></label></div></section>
    {!isAdmin && <><section><h3 className="font-semibold">Permissões do sistema</h3><p className="mb-3 text-sm text-slate-500">Marque somente o que este funcionário pode visualizar e utilizar.</p><div className="grid gap-4 md:grid-cols-3">{permissionGroups.map((group) => <fieldset className="rounded-xl border border-slate-200 p-3" key={group.title}><legend className="px-1 text-sm font-semibold">{group.title}</legend><div className="grid gap-2">{group.items.map(([value, label]) => <label className="flex items-center gap-2 text-sm" key={value}><input type="checkbox" checked={form.permissions.includes(value)} onChange={() => toggle("permissions", value)} />{label}</label>)}</div></fieldset>)}</div></section><section><h3 className="font-semibold">Caixas autorizados</h3><p className="mb-3 text-sm text-slate-500">O funcionário verá apenas os caixas marcados ao iniciar o atendimento.</p><div className="grid gap-2 sm:grid-cols-2">{registers.map((register) => <label className="flex items-center gap-2 rounded-lg border border-slate-200 p-3 text-sm" key={register.id}><input type="checkbox" checked={form.cashRegisterIds.includes(register.id)} onChange={() => toggle("cashRegisterIds", register.id)} /><span><b>{register.name}</b><br /><span className="text-xs text-slate-500">Código {register.code}</span></span></label>)}{!registers.length && <p className="text-sm text-amber-700">Cadastre os caixas no Financeiro antes de autorizar o funcionário.</p>}</div></section></>}
    <label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} />Acesso ativo</label>{error && <p className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}<button className="btn btn-primary justify-self-end">Salvar funcionário</button>
  </form></Modal>;
}

function TeamUserModalLegacy({ user, registers, onClose, onSaved }: { user: TeamUser | null; registers: CashRegister[]; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ name: user?.name ?? "", email: user?.email ?? "", password: "", role: user?.role ?? "EMPLOYEE", active: user?.active ?? true, permissions: user?.permissions ?? [], cashRegisterIds: user?.cashRegisterAccess.map((access) => access.cashRegisterId) ?? [] }); const [error, setError] = useState(""); const isAdmin = form.role === "ADMIN";
  const toggle = (field: "permissions" | "cashRegisterIds", value: string) => setForm((current) => ({ ...current, [field]: current[field].includes(value) ? current[field].filter((item) => item !== value) : [...current[field], value] }));
  async function submit(event: React.FormEvent) { event.preventDefault(); setError(""); try { await api(user ? `/management/team/${user.id}` : "/management/team", { method: user ? "PATCH" : "POST", body: JSON.stringify({ ...form, password: form.password || undefined }) }); onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível salvar o funcionário."); } }
  return <Modal title={user ? "Editar funcionário" : "Cadastrar funcionário"} onClose={onClose}><form className="grid gap-5" onSubmit={submit}><section className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">Nome completo<input className="field mt-1" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label className="text-sm font-semibold">E-mail de acesso<input className="field mt-1" required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label><label className="text-sm font-semibold">{user ? "Nova senha (deixe vazia para manter)" : "Senha para entrar"}<input className="field mt-1" required={!user} minLength={6} type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></label><label className="text-sm font-semibold">Tipo de acesso<select className="field mt-1" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as "ADMIN" | "EMPLOYEE" })}><option value="EMPLOYEE">Funcionário — acesso personalizado</option><option value="ADMIN">Administrador — acesso completo</option></select></label></section>{!isAdmin && <><section><h3 className="font-semibold">Permissões do sistema</h3><p className="mb-3 text-sm text-slate-500">Marque somente o que este funcionário pode visualizar e utilizar.</p><div className="grid gap-4 md:grid-cols-3">{permissionGroups.map((group) => <fieldset className="rounded-xl border border-slate-200 p-3" key={group.title}><legend className="px-1 text-sm font-semibold">{group.title}</legend><div className="grid gap-2">{group.items.map(([value, label]) => <label className="flex items-center gap-2 text-sm" key={value}><input type="checkbox" checked={form.permissions.includes(value)} onChange={() => toggle("permissions", value)} />{label}</label>)}</div></fieldset>)}</div></section><section><h3 className="font-semibold">Caixas autorizados</h3><p className="mb-3 text-sm text-slate-500">O funcionário verá apenas os caixas marcados ao iniciar o atendimento.</p><div className="grid gap-2 sm:grid-cols-2">{registers.map((register) => <label className="flex items-center gap-2 rounded-lg border border-slate-200 p-3 text-sm" key={register.id}><input type="checkbox" checked={form.cashRegisterIds.includes(register.id)} onChange={() => toggle("cashRegisterIds", register.id)} /><span><b>{register.name}</b><br /><span className="text-xs text-slate-500">Código {register.code}</span></span></label>)}{!registers.length && <p className="text-sm text-amber-700">Cadastre os caixas no Financeiro antes de autorizar o funcionário.</p>}</div></section></>}<label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} />Acesso ativo</label>{error && <p className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}<button className="btn btn-primary justify-self-end">Salvar funcionário</button></form></Modal>;
}
...
function FinancialCashRegisters() {
  const { data: registers, refresh } = useData<CashRegister[]>("/management/cash-registers");
  const [editing, setEditing] = useState<CashRegister | null>(null); const [open, setOpen] = useState(false);
  return <Page title="Caixas" action={<button className="btn btn-primary" onClick={() => { setEditing(null); setOpen(true); }}><Plus size={16} />Criar caixa</button>}>
    <p className="mb-4 text-sm text-slate-600">Cadastre os caixas físicos da empresa. O saldo é individual e cada funcionário verá somente os caixas autorizados para ele.</p>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{(registers ?? []).map((register) => <div className="panel p-5" key={register.id}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase text-slate-500">Caixa {register.code}</p><h2 className="mt-1 text-lg font-semibold">{register.name}</h2></div><span className={`badge ${register.active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{register.active ? "Ativo" : "Inativo"}</span></div><p className="mt-4 text-sm text-slate-500">Dinheiro atual</p><b className="block text-2xl">{currency(register.balance ?? register.initialBalance)}</b><p className="mt-3 text-xs text-slate-500">{register.openSession ? `Aberto por ${register.openSession.openedByName ?? "usuário"}` : "Fechado"}</p><p className="mt-1 text-xs text-slate-500">Autorizados: {register.allowedUsers?.filter((user) => user.active).map((user) => user.name).join(", ") || "Somente administradores"}</p><div className="mt-4 flex gap-2"><button className="btn btn-secondary" onClick={() => { setEditing(register); setOpen(true); }}>Editar</button><button className="btn btn-secondary text-red-700" onClick={async () => { if (!window.confirm(`Excluir o caixa ${register.name}?`)) return; await api(`/management/cash-registers/${register.id}`, { method: "DELETE" }); refresh(); }}>Excluir</button></div></div>)}{!registers?.length && <div className="panel p-6 text-sm text-slate-500">Nenhum caixa cadastrado.</div>}</div>
    {open && <CashRegisterModal register={editing} onClose={() => { setOpen(false); setEditing(null); }} onSaved={() => { setOpen(false); setEditing(null); refresh(); }} />}
  </Page>;
}

function CashRegisterModal({ register, onClose, onSaved }: { register: CashRegister | null; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ code: register?.code ?? "", name: register?.name ?? "", initialBalance: String(register?.initialBalance ?? "0"), active: register?.active ?? true, notes: register?.notes ?? "" }); const [error, setError] = useState("");
  async function submit(event: React.FormEvent) { event.preventDefault(); setError(""); try { await api(register ? `/management/cash-registers/${register.id}` : "/management/cash-registers", { method: register ? "PATCH" : "POST", body: JSON.stringify({ ...form, initialBalance: Number(form.initialBalance.replace(",", ".")) }) }); onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível salvar o caixa."); } }
  return <Modal title={register ? "Editar caixa" : "Criar novo caixa"} onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}><label className="text-sm font-semibold">Número ou código<input className="field mt-1" required placeholder="Ex.: 401" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} /></label><label className="text-sm font-semibold">Nome do caixa<input className="field mt-1" required placeholder="Ex.: Caixa Recepção" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label className="text-sm font-semibold">Dinheiro inicial<input className="field mt-1" inputMode="decimal" value={form.initialBalance} onChange={(e) => setForm({ ...form, initialBalance: e.target.value })} /></label><label className="flex items-center gap-2 self-end rounded-lg bg-slate-50 p-3 text-sm font-semibold"><input type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} />Caixa ativo</label><label className="text-sm font-semibold sm:col-span-2">Observações<textarea className="field mt-1" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></label>{error && <p className="text-sm text-red-700 sm:col-span-2">{error}</p>}<button className="btn btn-primary justify-self-end sm:col-span-2">Salvar caixa</button></form></Modal>;
}

function useFinancialDates(currentMonth = false) {
  const today = localDateInput(); const base = new Date(`${today}T12:00:00`); const future = new Date(base); future.setDate(future.getDate() + 30);
  const monthStart = `${base.getFullYear()}-${String(base.getMonth() + 1).padStart(2, "0")}-01`;
  const [from, setFrom] = useState(currentMonth ? monthStart : today); const [to, setTo] = useState(currentMonth ? today : `${future.getFullYear()}-${String(future.getMonth() + 1).padStart(2, "0")}-${String(future.getDate()).padStart(2, "0")}`);
  return { from, to, setFrom, setTo };
}

function FinancialReceivables({ accounts }: { accounts: FinancialAccount[] }) {
  const dates = useFinancialDates(); const [status, setStatus] = useState("ALL"); const [accountId, setAccountId] = useState("");
  const query = `/financial/receivables?from=${dates.from}&to=${dates.to}&status=${status}${accountId ? `&accountId=${accountId}` : ""}`;
  const { data, loading } = useData<{ items: FinancialReceivable[]; summary: { gross: number; fees: number; net: number; count: number } }>(query);
  return <Page title="Recebimentos previstos"><p className="mb-4 text-sm text-slate-600">Veja quando cada parcela de PIX ou cartão entrará no banco, já descontando as taxas configuradas.</p>
    <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><label className="text-sm font-semibold">Data inicial<input className="field mt-1" type="date" value={dates.from} onChange={(e) => dates.setFrom(e.target.value)} /></label><label className="text-sm font-semibold">Data final<input className="field mt-1" type="date" value={dates.to} onChange={(e) => dates.setTo(e.target.value)} /></label><label className="text-sm font-semibold">Situação<select className="field mt-1" value={status} onChange={(e) => setStatus(e.target.value)}><option value="ALL">Todos</option><option value="EXPECTED">Ainda vai receber</option><option value="RECEIVED">Já entrou no banco</option></select></label><label className="text-sm font-semibold">Banco<select className="field mt-1" value={accountId} onChange={(e) => setAccountId(e.target.value)}><option value="">Todos os bancos</option>{accounts.filter((item) => item.type !== "CASH_DRAWER").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
    <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-xl border border-blue-200 bg-blue-50 p-4"><p className="text-sm text-blue-700">Valor das vendas</p><b className="text-xl">{currency(data?.summary.gross ?? 0)}</b></div><div className="rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm text-amber-700">Taxas descontadas</p><b className="text-xl">{currency(data?.summary.fees ?? 0)}</b></div><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-sm text-emerald-700">Valor líquido no banco</p><b className="text-xl">{currency(data?.summary.net ?? 0)}</b></div><div className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-sm text-slate-600">Parcelas encontradas</p><b className="text-xl">{data?.summary.count ?? 0}</b></div></div>
    <div className="grid gap-3">{(data?.items ?? []).map((item) => <div className="panel flex flex-col gap-3 p-4 lg:flex-row lg:items-center lg:justify-between" key={item.id}><div><div className="flex flex-wrap items-center gap-2"><b>{dateBR(item.expectedDate)} · {item.methodName}{item.brand ? ` · ${item.brand}` : ""}</b><span className={`badge ${item.status === "RECEIVED" ? "bg-emerald-50 text-emerald-700" : "bg-blue-50 text-blue-700"}`}>{item.status === "RECEIVED" ? "Recebido" : "Previsto"}</span></div><p className="mt-1 text-sm text-slate-600">Venda {item.saleCode ?? "-"} · {item.customerName ?? "Cliente não informado"} · Parcela {item.installmentNumber}/{item.installments}</p><p className="text-sm text-slate-500">Destino: {item.account?.name ?? "Banco não informado"}{item.nsu ? ` · NSU ${item.nsu}` : ""}</p></div><div className="grid grid-cols-3 gap-4 text-right text-sm"><span>Bruto<br /><b>{currency(item.grossAmount)}</b></span><span>Taxa<br /><b className="text-amber-700">-{currency(item.feeAmount)}</b></span><span>Líquido<br /><b className="text-emerald-700">{currency(item.netAmount)}</b></span></div></div>)}{!loading && !data?.items.length && <div className="panel p-8 text-center text-sm text-slate-500">Nenhum recebimento encontrado neste período.</div>}</div>
  </Page>;
}

function FinancialMovements({ accounts }: { accounts: FinancialAccount[] }) {
  const dates = useFinancialDates(true); const [accountId, setAccountId] = useState("");
  const { data, loading } = useData<{ items: FinancialMovement[]; summary: { incoming: number; outgoing: number; balance: number } }>(`/financial/movements?from=${dates.from}&to=${dates.to}${accountId ? `&accountId=${accountId}` : ""}`);
  return <Page title="Movimentações"><p className="mb-4 text-sm text-slate-600">Extrato consolidado das entradas recebidas e das contas pagas.</p><div className="mb-4 grid gap-3 sm:grid-cols-3"><label className="text-sm font-semibold">Data inicial<input className="field mt-1" type="date" value={dates.from} onChange={(e) => dates.setFrom(e.target.value)} /></label><label className="text-sm font-semibold">Data final<input className="field mt-1" type="date" value={dates.to} onChange={(e) => dates.setTo(e.target.value)} /></label><label className="text-sm font-semibold">Conta<select className="field mt-1" value={accountId} onChange={(e) => setAccountId(e.target.value)}><option value="">Todas as contas e caixas</option>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div><div className="mb-4 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-sm text-emerald-700">Entradas</p><b className="text-xl">{currency(data?.summary.incoming ?? 0)}</b></div><div className="rounded-xl border border-red-200 bg-red-50 p-4"><p className="text-sm text-red-700">Saídas</p><b className="text-xl">{currency(data?.summary.outgoing ?? 0)}</b></div><div className={`rounded-xl border p-4 ${(data?.summary.balance ?? 0) >= 0 ? "border-blue-200 bg-blue-50" : "border-red-200 bg-red-50"}`}><p className="text-sm">Resultado do período</p><b className="text-xl">{currency(data?.summary.balance ?? 0)}</b></div></div><div className="panel overflow-hidden"><div className="divide-y divide-slate-100">{(data?.items ?? []).map((item) => <div className="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between" key={item.id}><div><b>{dateTimeBR(item.date)} · {item.account}</b><p className="text-sm text-slate-600">{item.description}</p></div><b className={item.direction === "IN" ? "text-emerald-700" : "text-red-700"}>{item.direction === "IN" ? "+" : "-"}{currency(item.amount)}</b></div>)}{!loading && !data?.items.length && <p className="p-8 text-center text-sm text-slate-500">Nenhuma movimentação encontrada.</p>}</div></div></Page>;
}
...
function FinancialModule({ sectionPage }: { sectionPage: string }) {
  ...
  const { data: overview, refresh: refreshOverview } = useData<{ payable: number; receivable: number; fees: number }>("/financial/overview");
  const [accountOpen, setAccountOpen] = useState(false);
  const [editingAccount, setEditingAccount] = useState<FinancialAccount | null>(null);
  const [methodOpen, setMethodOpen] = useState(false);
  const [editingMethod, setEditingMethod] = useState<FinancialPaymentMethod | null>(null);
  const hasActiveBank = Boolean(accounts?.some((account) => account.active && account.type !== "CASH_DRAWER"));
  if (sectionPage === "financial:cash-registers") return <FinancialCashRegisters />;
  if (sectionPage === "financial:accounts") return <Page title="Bancos" action={<button className="btn btn-primary" onClick={() => { setEditingAccount(null); setAccountOpen(true); }}><Plus size={16} />Cadastrar banco</button>}>
    <p className="mb-4 text-sm text-slate-600">Cadastre somente as contas bancárias onde serão recebidos PIX, cartões e valores das maquininhas. Os caixas físicos são administrados no submenu Caixas.</p>
    <DataCards items={(accounts ?? []).filter((account) => account.type !== "CASH_DRAWER").map((account) => ({ title: account.name, subtitle: `${financialAccountTypeLabels[account.type]}${account.institutionName ? ` · ${account.institutionName}` : ""}`, meta: `Saldo inicial: ${currency(account.openingBalance)} | Saldo calculado: ${currency(account.calculatedBalance ?? account.openingBalance)}${account.isPrimary ? " | Conta principal" : ""}`, status: "Ativa", action: <div className="flex flex-wrap gap-2"><button className="btn btn-secondary" onClick={() => { setEditingAccount(account); setAccountOpen(true); }}>Editar</button><button className="btn btn-secondary text-red-700" onClick={async () => { if (!window.confirm(`Excluir o banco ${account.name}?`)) return; await api(`/financial/accounts/${account.id}`, { method: "DELETE" }); refreshAccounts(); refreshMethods(); }}>Excluir</button></div> }))} />
    {accountOpen && <FinancialAccountModal account={editingAccount} onClose={() => { setAccountOpen(false); setEditingAccount(null); }} onSaved={() => { setAccountOpen(false); setEditingAccount(null); refreshAccounts(); }} />}
  </Page>;
  if (sectionPage === "financial:methods") return <Page title="Formas de recebimento" action={<button className="btn btn-primary" disabled={!hasActiveBank} onClick={() => { setEditingMethod(null); setMethodOpen(true); }}><Plus size={16} />Nova forma</button>}>
    {!hasActiveBank && <p className="mb-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">Cadastre um banco ativo no submenu Bancos antes de criar formas de recebimento.</p>}
  ...
  if (sectionPage === "financial:receivables") return <FinancialReceivables accounts={accounts ?? []} />;
  if (sectionPage === "financial:payables") return <FinancialPayables accounts={accounts ?? []} onChanged={() => { refreshAccounts(); refreshOverview(); }} />;
  if (sectionPage === "financial:movements") return <FinancialMovements accounts={accounts ?? []} />;
  if (sectionPage === "financial:reports") return <FinancialReports />;
...
function FinancialPayables({ accounts, onChanged }: { accounts: FinancialAccount[]; onChanged: () => void }) {
  ...
  const [customTo, setCustomTo] = useState(localDateInput());
  const range = useMemo(() => {
    const today = new Date(`${localDateInput()}T12:00:00`);
    const format = (date: Date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
    if (period === "CUSTOM") return { from: customFrom, to: customTo };
    const end = new Date(today);
    if (period === "10_DAYS") end.setDate(end.getDate() + 10);
    if (period === "WEEK") end.setDate(end.getDate() + 7);
    if (period === "MONTH") end.setMonth(end.getMonth() + 1);
    if (period === "YEAR") end.setFullYear(end.getFullYear() + 1);
    return { from: status === "OPEN" ? "" : format(today), to: format(end) };
  }, [period, customFrom, customTo, status]);
  const query = `/financial/payables?status=${status}${range.from ? `&from=${range.from}` : ""}&to=${range.to}`;
  const { data: payables, refresh } = useData<FinancialPayable[]>(query);
  ...
  return <Page title="Contas a pagar" action={<button className="btn btn-primary" onClick={() => { setEditing(null); setFormOpen(true); }}><Plus size={16} />Nova conta</button>}>
    ...
    {period === "CUSTOM" && <div className="panel mb-4 grid gap-3 p-4 sm:grid-cols-2"><label className="text-sm font-semibold">Data inicial<input className="field mt-1" type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} /></label><label className="text-sm font-semibold">Data final<input className="field mt-1" type="date" min={customFrom} value={customTo} onChange={(e) => setCustomTo(e.target.value)} /></label></div>}
    <div className="grid gap-3">{(payables ?? []).map((item) => { const isOverdue = item.status === "OPEN" && new Date(item.dueDate) < new Date(); return <div className="panel grid gap-3 p-4 lg:grid-cols-[1fr_170px_160px_auto] lg:items-center" key={item.id}><div><div className="flex flex-wrap items-center gap-2"><b>{item.description}</b><span className={`rounded-full px-2 py-0.5 text-xs ${isOverdue ? "bg-red-100 text-red-700" : item.status === "PAID" ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"}`}>{isOverdue ? "Vencida" : item.status === "PAID" ? "Paga" : item.status === "CANCELLED" ? "Cancelada" : "Em aberto"}</span><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs">{item.expenseType === "FIXED" ? "Fixa" : "Variável"}</span></div><p className="mt-1 text-sm text-slate-600">{payableCategoryLabels[item.category] ?? item.category}{item.supplierName ? ` · ${item.supplierName}` : ""}{item.documentNumber ? ` · Documento ${item.documentNumber}` : ""}</p>{item.installmentTotal > 1 && <p className="text-xs text-slate-500">Parcela {item.installmentNumber}/{item.installmentTotal}</p>}</div><div><p className="text-xs text-slate-500">Vencimento</p><b>{dateBR(item.dueDate)}</b></div><div><p className="text-xs text-slate-500">Valor</p><b className="text-lg">{currency(item.amount)}</b></div><div className="flex flex-wrap justify-end gap-2">{item.status === "OPEN" && <><button className="btn btn-primary" onClick={() => setPaying(item)}>Pagar</button><button className="btn btn-secondary" onClick={() => { setEditing(item); setFormOpen(true); }}>Editar</button><button className="btn btn-secondary text-red-700" onClick={async () => { if (!window.confirm(`Excluir a conta ${item.description}?`)) return; await api(`/financial/payables/${item.id}`, { method: "DELETE" }); refresh(); }}>Excluir</button></>}{item.status === "PAID" && <p className="text-sm text-slate-600">Pago em {item.paidAt ? dateBR(item.paidAt) : "-"}<br />{item.paidFromAccount?.name ?? ""}</p>}</div></div>; })}{!payables?.length && <div className="panel p-8 text-center text-slate-500">Nenhuma conta encontrada.</div>}</div>
    {formOpen && <PayableModal payable={editing} onClose={() => { setFormOpen(false); setEditing(null); }} onSaved={() => { setFormOpen(false); setEditing(null); refresh(); onChanged(); }} />}
    {paying && <PayablePaymentModal payable={paying} accounts={accounts} onClose={() => setPaying(null)} onSaved={() => { setPaying(null); refresh(); onChanged(); }} />}
  </Page>;
}
...
function PayableModal({ payable, onClose, onSaved }: { payable: FinancialPayable | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={payable ? "Editar conta a pagar" : "Nova conta a pagar"} onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
    ...
    <label className="text-sm font-semibold">Número do boleto ou nota fiscal <span className="font-normal text-slate-500">(opcional)</span><input className="field mt-1" value={form.documentNumber} onChange={(e) => setForm({ ...form, documentNumber: e.target.value })} /></label>
    <label className="text-sm font-semibold sm:col-span-2">Observações <span className="font-normal text-slate-500">(opcional)</span><textarea className="field mt-1" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></label>
    {error && <p className="rounded-md bg-red-50 p-3 text-sm text-red-700 sm:col-span-2">{error}</p>}<button className="btn btn-primary justify-self-end sm:col-span-2" disabled={saving}>{saving ? "Salvando..." : "Salvar conta"}</button>
  </form></Modal>;
}

function PayablePaymentModal({ payable, accounts, onClose, onSaved }: { payable: FinancialPayable; accounts: FinancialAccount[]; onClose: () => void; onSaved: () => void }) {
  const overdue = new Date(`${payable.dueDate.slice(0, 10)}T23:59:59`) < new Date();
  const [form, setForm] = useState({ paidAmount: String(payable.amount), paidAt: localDateInput(), paidFromAccountId: accounts[0]?.id ?? "", paymentMethod: "PIX", hasInterest: false, receiptFileName: "", receiptMimeType: "", receiptData: "" }); const [error, setError] = useState(""); const [saving, setSaving] = useState(false);
  const interest = Math.max(0, Number(form.paidAmount.replace(",", ".")) - Number(payable.amount));
  async function chooseReceipt(file?: File) { if (!file) return setForm({ ...form, receiptFileName: "", receiptMimeType: "", receiptData: "" }); if (file.size > 2_000_000) return setError("O comprovante deve ter no máximo 2 MB."); const reader = new FileReader(); reader.onload = () => setForm((current) => ({ ...current, receiptFileName: file.name, receiptMimeType: file.type, receiptData: String(reader.result) })); reader.readAsDataURL(file); }
  async function submit(event: React.FormEvent) { event.preventDefault(); setError(""); setSaving(true); try { await api(`/financial/payables/${payable.id}/pay`, { method: "POST", body: JSON.stringify({ ...form, paidAmount: Number(form.paidAmount.replace(",", ".")) }) }); onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível registrar o pagamento."); } finally { setSaving(false); } }
  return <Modal title="Pagar conta" onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
    <div className={`rounded-lg p-3 sm:col-span-2 ${overdue ? "bg-red-50 text-red-900" : "bg-slate-50"}`}><b>{payable.description} · {payable.recurrenceIndefinite ? `Pagamento mensal ${payable.installmentNumber}` : `Parcela ${payable.installmentNumber}/${payable.installmentTotal}`}</b><p>Valor original: {currency(payable.amount)} · Vencimento: {dateBR(payable.dueDate)}</p>{overdue && <p className="mt-1 font-semibold">Esta conta está vencida.</p>}</div>
    <fieldset className="rounded-lg border border-slate-200 p-3 sm:col-span-2"><legend className="px-1 text-sm font-semibold">Houve juros ou multa?</legend><div className="mt-2 flex gap-6"><label className="flex items-center gap-2"><input type="radio" checked={!form.hasInterest} onChange={() => setForm({ ...form, hasInterest: false, paidAmount: String(payable.amount) })} />Não</label><label className="flex items-center gap-2"><input type="radio" checked={form.hasInterest} onChange={() => setForm({ ...form, hasInterest: true })} />Sim</label></div>{form.hasInterest && <label className="mt-3 block text-sm font-semibold">Valor total pago, já com juros<input className="field mt-1" required inputMode="decimal" value={form.paidAmount} onChange={(e) => setForm({ ...form, paidAmount: e.target.value })} /><span className="mt-1 block text-sm font-normal text-amber-700">Juros e multa calculados: {currency(interest)}</span></label>}</fieldset>
    {!form.hasInterest && <label className="text-sm font-semibold">Valor pago<input className="field mt-1 bg-slate-50" readOnly value={currency(payable.amount)} /></label>}
    <label className="text-sm font-semibold">Data do pagamento<input className="field mt-1" required type="date" value={form.paidAt} onChange={(e) => setForm({ ...form, paidAt: e.target.value })} /></label>
    <label className="text-sm font-semibold">Dinheiro saiu de qual conta?<select className="field mt-1" required value={form.paidFromAccountId} onChange={(e) => setForm({ ...form, paidFromAccountId: e.target.value })}><option value="">Selecione...</option>{accounts.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select></label>
    <label className="text-sm font-semibold">Como foi pago?<select className="field mt-1" value={form.paymentMethod} onChange={(e) => setForm({ ...form, paymentMethod: e.target.value })}><option>PIX</option><option>Dinheiro</option><option>Débito automático</option><option>Boleto</option><option>Transferência</option><option>Cartão</option><option>Outro</option></select></label>
    <label className="text-sm font-semibold sm:col-span-2">Anexar comprovante <span className="font-normal text-slate-500">(PDF, foto ou imagem — máximo 2 MB)</span><input className="field mt-1" type="file" accept="image/*,.pdf" onChange={(e) => chooseReceipt(e.target.files?.[0])} />{form.receiptFileName && <span className="mt-1 block text-sm font-normal text-emerald-700">Arquivo selecionado: {form.receiptFileName}</span>}</label>
    {error && <p className="rounded-md bg-red-50 p-3 text-sm text-red-700 sm:col-span-2">{error}</p>}<button className="btn btn-primary justify-self-end sm:col-span-2" disabled={saving}>{saving ? "Registrando..." : "Confirmar pagamento"}</button>
  </form></Modal>;
}

function FinancialAccountModal({ account, onClose, onSaved }: { account: FinancialAccount | null; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({ name: account?.name ?? "", type: account?.type ?? "CHECKING_ACCOUNT", institutionName: account?.institutionName ?? "", agency: account?.agency ?? "", accountNumber: account?.accountNumber ?? "", internalIdentifier: account?.internalIdentifier ?? "", openingBalance: String(account?.openingBalance ?? "0"), openingBalanceDate: account?.openingBalanceDate?.slice(0, 10) ?? localDateInput(), isPrimary: account?.isPrimary ?? false, active: true, notes: account?.notes ?? "" });
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const isCashDrawer = form.type === "CASH_DRAWER";
  async function submit(event: React.FormEvent) { event.preventDefault(); setSaving(true); setError(""); try { const openingBalance = Number(form.openingBalance.replace(",", ".")); const payload = isCashDrawer ? { name: form.name, openingBalance, openingBalanceDate: form.openingBalanceDate, notes: form.notes } : { ...form, openingBalance }; await api(account ? `/financial/accounts/${account.id}` : "/financial/accounts", { method: account ? "PATCH" : "POST", body: JSON.stringify(payload) }); onSaved(); } catch (e) { setError(e instanceof Error ? e.message : "Não foi possível salvar a conta."); } finally { setSaving(false); } }
  return <Modal title={account ? `Editar — ${account.name}` : "Cadastrar banco"} onClose={onClose}><form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
    <label className="text-sm font-semibold">Nome para identificar a conta<input className="field mt-1" required placeholder="Ex.: Santander da empresa" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label>
    {!isCashDrawer && <><label className="text-sm font-semibold">Banco ou instituição<input className="field mt-1" required placeholder="Ex.: Santander, Itaú, Nubank" value={form.institutionName} onChange={(e) => setForm({ ...form, institutionName: e.target.value })} /></label>
    <label className="text-sm font-semibold">Tipo de conta<select className="field mt-1" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as FinancialAccount["type"] })}>{Object.entries(financialAccountTypeLabels).filter(([value]) => value !== "CASH_DRAWER").map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
    <label className="text-sm font-semibold">Agência <span className="font-normal text-slate-500">(opcional)</span><input className="field mt-1" placeholder="Ex.: 1234" value={form.agency} onChange={(e) => setForm({ ...form, agency: e.target.value })} /></label>
    <label className="text-sm font-semibold">Número da conta <span className="font-normal text-slate-500">(opcional)</span><input className="field mt-1" placeholder="Ex.: 12345-6" value={form.accountNumber} onChange={(e) => setForm({ ...form, accountNumber: e.target.value })} /></label>
    <label className="text-sm font-semibold">Identificação interna <span className="font-normal text-slate-500">(opcional)</span><input className="field mt-1" placeholder="Ex.: Conta Getnet" value={form.internalIdentifier} onChange={(e) => setForm({ ...form, internalIdentifier: e.target.value })} /></label></>}
    <label className="text-sm font-semibold">Saldo atual da conta (R$)<input className="field mt-1" inputMode="decimal" placeholder="0,00" value={form.openingBalance} onChange={(e) => setForm({ ...form, openingBalance: e.target.value })} /><small className="mt-1 block font-normal text-slate-500">Valor existente antes de começar a usar o sistema.</small></label>
    <label className="text-sm font-semibold">Data do saldo informado<input className="field mt-1" type="date" value={form.openingBalanceDate} onChange={(e) => setForm({ ...form, openingBalanceDate: e.target.value })} /></label>
    {!isCashDrawer && <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isPrimary} onChange={(e) => setForm({ ...form, isPrimary: e.target.checked })} />Usar como conta bancária principal</label>}
    <label className="text-sm font-semibold sm:col-span-2">Observações <span className="font-normal text-slate-500">(opcional)</span><textarea className="field mt-1" placeholder="Informações adicionais sobre esta conta" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></label>
    {error && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700 sm:col-span-2">{error}</p>}<button className="btn btn-primary justify-self-end sm:col-span-2" disabled={saving}>{saving ? "Salvando..." : account ? "Salvar alterações" : "Cadastrar banco"}</button>
  </form></Modal>;
}

function FinancialMethodModal({ method, accounts, onClose, onSaved }: { method: FinancialPaymentMethod | null; accounts: FinancialAccount[]; onClose: () => void; onSaved: () => void }) {
  type RateRow = { fee: string; anticipates: boolean; anticipationFee: string };
  const [form, setForm] = useState({ name: method?.name ?? "", type: method?.type ?? "PIX", institutionName: method?.institutionName ?? "", destinationAccountId: method?.destinationAccountId ?? accounts.find((account) => account.type !== "CASH_DRAWER")?.id ?? "", maxInstallments: method?.maxInstallments ?? 1, brands: method?.brands.filter((brand) => brand.active).map((brand) => brand.brandName).join(", ") ?? "", settlementContract: method?.settlementContract ?? "SCHEDULED", settlementDays: String(method?.settlementDays ?? 1), settlementDayType: method?.settlementDayType ?? "BUSINESS" });
  const [rates, setRates] = useState<RateRow[]>(() => Array.from({ length: method?.maxInstallments ?? 1 }, (_, index) => { const rule = method?.feeRules.find((item) => item.active && (item.installments ?? 1) === index + 1); return { fee: String(rule?.feePercentage ?? method?.defaultFeePercentage ?? ""), anticipates: Boolean(rule?.allowsAnticipation ?? method?.allowsAnticipation), anticipationFee: String(rule?.anticipationFeePercentage ?? method?.anticipationFeePercentage ?? "") }; }));
  const [error, setError] = useState("");
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (isCash) {
      try { await api(`/financial/payment-methods/${method?.id}`, { method: "PATCH", body: JSON.stringify({ name: form.name, destinationAccountId: form.destinationAccountId }) }); onSaved(); }
      catch (e) { setError(e instanceof Error ? e.message : "Não foi possível editar a forma Dinheiro."); }
      return;
    }
    ...
    try {
      ...
      const methodPayload = { name: form.name, type: form.type, institutionName: form.institutionName, destinationAccountId: form.destinationAccountId, defaultFeePercentage: Number((rates[0]?.fee || "0").replace(",", ".")), fixedFee: 0, allowsAnticipation: sameDayCredit, anticipationFeePercentage: sameDayCredit ? Number((rates[0]?.anticipationFee || "0").replace(",", ".")) : 0, settlementDays, settlementDayType: form.settlementDayType, settlementContract: form.settlementContract, maxInstallments: isCredit ? form.maxInstallments : 1, requiresNsu: isCard, requiresReceiptCode: false, active: true, brands: isCard ? form.brands.split(",").map((item) => item.trim()).filter(Boolean) : [] };
      const created = await api<FinancialPaymentMethod>(method ? `/financial/payment-methods/${method.id}` : "/financial/payment-methods", { method: method ? "PATCH" : "POST", body: JSON.stringify(methodPayload) });
  ...
  return <Modal title={method ? `Editar — ${method.name}` : "Nova forma de pagamento"} onClose={onClose}><form className="grid gap-5" onSubmit={submit}>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-semibold">Tipo de pagamento<select className="field mt-1" disabled={isCash} value={form.type} onChange={(e) => { const type = e.target.value as FinancialPaymentMethod["type"]; const destinationAccountId = accounts.find((account) => type === "CASH" ? account.type === "CASH_DRAWER" : account.type !== "CASH_DRAWER")?.id ?? ""; setForm({ ...form, type, maxInstallments: 1, destinationAccountId }); setRates([{ fee: "", anticipates: false, anticipationFee: "" }]); }}>{isCash && <option value="CASH">Dinheiro</option>}<option value="PIX">PIX</option><option value="DEBIT_CARD">Cartão de débito</option><option value="CREDIT_CARD">Cartão de crédito</option></select></label>
      {!isCash && <label className="text-sm font-semibold">Instituição ou maquininha<input className="field mt-1" placeholder="Ex.: Getnet, InfinitePay, Cielo" value={form.institutionName} onChange={(e) => setForm({ ...form, institutionName: e.target.value })} /></label>}
      <label className="text-sm font-semibold">Banco/conta que receberá o valor<select className="field mt-1" value={form.destinationAccountId} onChange={(e) => setForm({ ...form, destinationAccountId: e.target.value })}><option value="">Selecione</option>{accounts.filter((account) => isCash ? account.type === "CASH_DRAWER" : account.type !== "CASH_DRAWER").map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select><small className="mt-1 block font-normal text-slate-500">O sistema registrará nessa conta o valor líquido, depois das taxas.</small></label>
    ...
    {error && <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}<div className="flex justify-end"><button className="btn btn-primary">{method ? "Salvar alterações" : "Salvar forma de pagamento"}</button></div>
  </form></Modal>;
}
...
function FinancialMethodModalGrouped({ accounts, onClose, onSaved }: { accounts: FinancialAccount[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title="Nova forma de pagamento" onClose={onClose}><form className="grid gap-3 sm:grid-cols-2" onSubmit={submit}>
    <div className="rounded-lg bg-blue-50 p-3 sm:col-span-2"><h3 className="font-semibold text-blue-900">1. Identificação da forma de pagamento</h3><p className="mt-1 text-sm text-blue-800">Informe como essa opção será reconhecida pelos funcionários no caixa.</p></div>
    ...
    <div className="mt-2 rounded-lg bg-slate-100 p-3 sm:col-span-2"><h3 className="font-semibold">2. Taxas e prazo de recebimento</h3><p className="mt-1 text-sm text-slate-600">Esses valores normalmente aparecem no contrato da maquininha, banco ou operadora.</p></div>
    <label className="text-sm font-medium">Taxa percentual cobrada pela operadora (%)<input className="field mt-1" inputMode="decimal" placeholder="Ex.: 1,99" value={form.defaultFeePercentage} onChange={(e) => setForm({ ...form, defaultFeePercentage: e.target.value })} /><span className="mt-1 block text-xs font-normal text-slate-500">Percentual descontado sobre cada pagamento.</span></label>
    <label className="text-sm font-medium">Taxa fixa por pagamento (R$)<input className="field mt-1" inputMode="decimal" placeholder="Ex.: 0,50" value={form.fixedFee} onChange={(e) => setForm({ ...form, fixedFee: e.target.value })} /><span className="mt-1 block text-xs font-normal text-slate-500">Valor fixo descontado em cada transação.</span></label>
    ...
    <div className="mt-2 rounded-lg bg-slate-100 p-3 sm:col-span-2"><h3 className="font-semibold">3. Antecipação</h3><p className="mt-1 text-sm text-slate-600">Ative apenas se a operadora permitir receber antes do prazo normal.</p></div>
    <label className="flex items-start gap-2 text-sm"><input className="mt-1" type="checkbox" checked={form.allowsAnticipation} onChange={(e) => setForm({ ...form, allowsAnticipation: e.target.checked })} /><span><b>Permitir antecipação</b><small className="block text-slate-500">Permite receber antes da data normal, mediante uma taxa adicional.</small></span></label>
    {form.allowsAnticipation && <label className="text-sm font-medium">Taxa adicional de antecipação (%)<input className="field mt-1" inputMode="decimal" placeholder="Ex.: 2,50" value={form.anticipationFeePercentage} onChange={(e) => setForm({ ...form, anticipationFeePercentage: e.target.value })} /></label>}
    <label className="text-sm font-medium">Prazo para o dinheiro entrar na conta<input className="field mt-1" inputMode="numeric" placeholder="Quantidade de dias" value={form.settlementDays} onChange={(e) => setForm({ ...form, settlementDays: onlyDigits(e.target.value) })} /><span className="mt-1 block text-xs font-normal text-slate-500">Use 0 quando o recebimento for no mesmo dia.</span></label>
    <label className="text-sm font-medium">Como contar o prazo<select className="field mt-1" value={form.settlementDayType} onChange={(e) => setForm({ ...form, settlementDayType: e.target.value })}><option value="CALENDAR">Dias corridos — inclui fins de semana</option><option value="BUSINESS">Dias úteis — não inclui fins de semana</option></select></label>
    <div className="mt-2 rounded-lg bg-slate-100 p-3 sm:col-span-2"><h3 className="font-semibold">4. Informações exigidas no caixa</h3><p className="mt-1 text-sm text-slate-600">Escolha quais códigos o funcionário deverá informar ao receber o pagamento.</p></div>
    <label className="flex items-start gap-2 text-sm"><input className="mt-1" type="checkbox" checked={form.requiresNsu} onChange={(e) => setForm({ ...form, requiresNsu: e.target.checked })} /><span><b>Exigir NSU</b><small className="block text-slate-500">Código da transação informado pela maquininha.</small></span></label>
    <label className="flex items-start gap-2 text-sm"><input className="mt-1" type="checkbox" checked={form.requiresReceiptCode} onChange={(e) => setForm({ ...form, requiresReceiptCode: e.target.checked })} /><span><b>Exigir código do comprovante</b><small className="block text-slate-500">Obriga o operador a informar a identificação do comprovante.</small></span></label>
...
function Page({ title, action, children }: { title: string; action?: React.ReactNode; children: React.ReactNode }) {
  return <><div className="mb-4 flex flex-col gap-3 sm:mb-5 sm:flex-row sm:items-center sm:justify-between"><h1 className="text-xl font-semibold sm:text-2xl">{title}</h1>{action && <div className="grid [&>.btn]:w-full sm:block sm:[&>.btn]:w-auto">{action}</div>}</div>{children}</>;
}
...
function PublicCustomerRegistration({ token }: { token: string }) {
  const { data, error: linkError } = useData<{ companyName: string }>(`/public-registration/${token}`, [token]);
  const [form, setForm] = useState({ name: "", cpf: "", phone: "", zipCode: "", street: "", number: "", complement: "", neighborhood: "", city: "", state: "", petName: "", species: "DOG", customSpecies: "", breed: "", petGender: "UNINFORMED", size: "SMALL", color: "", notes: "" });
  const [saving, setSaving] = useState(false); const [error, setError] = useState(""); const [done, setDone] = useState(false); const [lookingUpCep, setLookingUpCep] = useState(false);
  async function lookupCep() { const cep = onlyDigits(form.zipCode, 8); if (cep.length !== 8) return; setLookingUpCep(true); try { const response = await fetch(`https://viacep.com.br/ws/${cep}/json/`); const address = await response.json(); if (address.erro) throw new Error(); setForm((current) => ({ ...current, street: address.logradouro ?? "", neighborhood: address.bairro ?? "", city: address.localidade ?? "", state: address.uf ?? "" })); } catch { setError("CEP não encontrado. Preencha o endereço manualmente."); } finally { setLookingUpCep(false); } }
  async function submit(event: React.FormEvent) { event.preventDefault(); setError(""); if (onlyDigits(form.cpf).length !== 11) return setError("Informe um CPF válido com 11 números."); setSaving(true); try { await api(`/public-registration/${token}`, { method: "POST", body: JSON.stringify({ ...form, cpf: onlyDigits(form.cpf, 11), phone: onlyDigits(form.phone, 11), zipCode: onlyDigits(form.zipCode, 8) }) }); setDone(true); } catch (err) { setError(err instanceof Error ? err.message : "Não foi possível concluir o cadastro."); } finally { setSaving(false); } }
  if (done) return <main className="grid min-h-screen place-items-center bg-slate-100 p-4"><section className="panel max-w-lg p-6 text-center sm:p-8"><div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-emerald-100 text-emerald-700"><Check size={30} /></div><h1 className="mt-4 text-2xl font-semibold">Cadastro concluído</h1><p className="mt-2 text-slate-600">Seus dados foram enviados com segurança para {data?.companyName ?? "a empresa"}.</p></section></main>;
  if (linkError) return <main className="grid min-h-screen place-items-center bg-slate-100 p-4"><section className="panel max-w-lg p-6 text-center"><h1 className="text-xl font-semibold">Link indisponível</h1><p className="mt-2 text-slate-600">{linkError}</p></section></main>;
...
export function App() {
  ...
  useEffect(() => {
    if (!session || session.user.role === "ADMIN" || page !== "dashboard" || session.user.permissions?.includes("dashboard")) return;
    setPage(session.user.permissions?.[0] ?? "dashboard");
  }, [session?.user.id]);
  useEffect(() => {
  ...
  const screen = useMemo(() => {
    const canAccess = (key: string) => session?.user.role === "ADMIN" || Boolean(session?.user.permissions?.includes(key));
    if (page === "team") return session?.user.role === "ADMIN" ? <TeamManagement /> : <Page title="Acesso negado"><p className="panel p-4 text-sm text-red-700">Somente administradores podem gerenciar a equipe.</p></Page>;
    if (!canAccess(page) && !(page.startsWith("checkout") && canAccess("checkout")) && !(page.startsWith("financial") && canAccess("financial"))) return <Page title="Acesso negado"><p className="panel p-4 text-sm text-red-700">Você não possui permissão para acessar esta área.</p></Page>;
    ...
    if (page.startsWith("financial")) return session?.user.role === "ADMIN" || canAccess(page)
